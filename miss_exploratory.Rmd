...
all_legs_med <- all_legs_small %>% 
  ...
  mutate(ten_sec_chunk = (time_in_sec%/%15)*15) %>% 
  group_by(ten_sec_chunk) %>% 
  summarise_at(6:23, median, na.rm = TRUE) %>% 
  mutate(date_time = as_datetime(ten_sec_chunk)) %>% 
  mutate(index = 1:n())
...
Download clean legs
```{r, eval=FALSE}
write_csv(all_legs_med, "clean_data\\clean_data_non_geom.csv")
BRRR::skrrrahh(0)
```
