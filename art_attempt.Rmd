...
clean_legs_non_geom <- clean_legs_non_geom %>% 
  mutate(chunk = chunk_num) %>% 
  filter(!is.na(chunk))


(first_per_chunk <- clean_legs_non_geom %>% 
  group_by(chunk) %>% 
  summarise(index = first(index), lat = first(lat), lon = first(lon)))

(last_per_chunk <- clean_legs_non_geom %>% 
  group_by(chunk) %>% 
...
first_last_chunk <- first_last_chunk %>% 
  mutate(grad_lat = gradient_top_edge - (gradient_top_edge - gradient_bottom_edge) * (index / nrow(clean_legs_non_geom)),
         grad_lon = gradient_right_edge) %>% 
...
(map <- ggplot() +
  ...
  geom_sf(data = clean_legs[seq(1, nrow(clean_legs), 30), ], aes(color = heading), alpha = 0.2, size = 2, show.legend = FALSE) +
...
(gradient <- clean_legs_non_geom[seq(1, nrow(clean_legs), 30), ] %>% 
  ...
  coord_cartesian(ylim = c(1, 25260/30),
                  xlim = c(-0.5, 0.5),
                  expand = FALSE))
...
ggsave("test_map.svg", plot = final_map, device = "svg")
```
